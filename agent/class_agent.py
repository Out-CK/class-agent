"""
Class Agent — top-level orchestrator for the full Class Run pipeline.

Same pipeline as ConcertAgent but tuned for class/workshop discovery.
Writes to the same event_entry_database with event_type='class'.
"""
...
class ClassAgent:
    ...
    def run(self) -> None:
        tracker = RunTracker(agent_name="class", run_type="web_search").start()
        run_start = time.time()
        ...
        logger.info(f"=== Class Run START | entry_batch_id={entry_batch_id} ===")
        ...
        try:
            ...
            tracker.set("pages_fetched_round1", len(seed_results))
        ...
        try:
            ...
            tracker.set("pages_fetched_round1", stats["pages_seed"] + stats["pages_round1"])
        ...
        # Step 4 — Find Additional Class Links
        ...
        try:
            ...
            for entry in raw_entries:
                ...
                entry.event_type = "class"  # enforce correct type
        ...
        self._step_log("Step 10: Insert Class Entries")
        try:
